...
} else{$breadcrumbs ="<a href='index.php'>Главная </a> / Корзина";}
...
if ($category_id == 0) {$arr = $con->queryObj("SELECT * FROM `honeyproducts`");} else  {
    if (cats_id($categories, $category_id)) {
      

        $arr = $con->queryObj("SELECT * FROM `honeyproducts`  WHERE  `category_id` IN($ids) ORDER BY `category`");} else { $arr = $con->queryObj("SELECT * FROM `honeyproducts` WHERE `category_id` = '$category_id'");}
